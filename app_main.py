...
# ======================== Markdown → PDF 转换器 (PDF export) ========================
def _sanitize_for_pdf(text: str) -> str:
    """Replace non-latin-1 punctuation so the core PDF font doesn't choke."""
    repl = {
        "\u2014": "-", "\u2013": "-",     # em / en dash
        "\u2026": "...",                   # ellipsis
        "\u2018": "'", "\u2019": "'",      # smart single quotes
    ...
    # Drop anything still outside latin-1 (e.g. emoji) rather than crash
    return text.encode("latin-1", "ignore").decode("latin-1")


def report_to_pdf_bytes(markdown_text: str, ticker: str, period: str) -> bytes:
    """Convert the Markdown report into downloadable PDF bytes (pure-Python)."""
    html = md_lib.markdown(
        _sanitize_for_pdf(markdown_text),
    ...
    pdf.cell(0, 10, _sanitize_for_pdf(f"{ticker} - Sentiment Analysis Report"),
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(110, 110, 110)
    pdf.cell(0, 6, _sanitize_for_pdf(f"Analysis period: {period}   |   Generated by DeepSent"),
             new_x="LMARGIN", new_y="NEXT")
...
# ==================== 下载按钮 (PDF) ====================
    try:
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"{selected_ticker}_Sentiment_Report_{start_date}_to_{end_date}.pdf",
            mime="application/pdf",
            use_container_width=True
        )
    except Exception as e:
        # Fallback so the user is never left without a download
        st.warning(f"PDF export failed ({e}); offering Markdown instead.")
        st.download_button(
            label="Download Full Report (Markdown)",
            data=cache["report"],
            file_name=f"{selected_ticker}_Sentiment_Report_{start_date}_to_{end_date}.md",
